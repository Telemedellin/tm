...
(function () {
	...
	a.lj = function (h) {
		...
		if (k.hh) {
			j = k.hh;
			if (typeof j == 'function') j = j.toString()
				.match(/function ([^(]+)/)[1];
			i += '?action=js&amp;func=' + j;
		}
		if (k.jx) {
	...
	function e(h, i, j) {
		i.on('appReady', function (k) {
			...
			h.folders = i.folders;
			h.files = i.ld['filesview.filesview'].data()
				.files;
	...
	d.prototype = {
		...
		getSelectedFile: function () {
			return b(this.id)
				.ld['filesview.filesview'].tools.dH();
		},
		getSelectedFiles: function () {
			return b(this.id)
				.ld['filesview.filesview'].tools.oO();
		},
		...
		filterFiles: function (h) {
			b(this.id)
				.ld['filesview.filesview'].oW('requestRenderFiles', {
					lookup: h
				});
		},
		...
		openDialog: function (h, i) {
			var l = this;
			var j = new a.dom.document(window.document)
				.getHead(),
				k = b(l.id)
				.document.getWindow();
			if (l.inFrame || l.inPopup || l.inIframe) a.document = b(l.id)
				.document;
			return b(l.id)
				.openDialog(h, i, j);
		},
		...
		openSkippedFilesDialog: function (h, i, j, k) {
			b(this.id)
				.skippedFilesDialog(h, i, j, k);
		},
		addTool: function (h) {
			return b(this.id)
				.plugins.tools.addTool(h);
		},
		addToolPanel: function (h) {
			return b(this.id)
				.plugins.tools.addToolPanel(h);
		},
		removeTool: function (h) {
			b(this.id)
				.plugins.tools.removeTool(h);
		},
		showTool: function (h) {
			b(this.id)
				.plugins.tools.showTool(h);
		},
		hideTool: function (h) {
			b(this.id)
				.plugins.tools.hideTool(h);
		},
		...
		resizeFormPanel: function (h) {
			...
			if (typeof h == 'undefined') {
				i.document.getById('panel_view')
					.setStyle('height', '');
				i.document.getById('panel_widget')
					.setStyle('height', '');
			} else {
				var j = Math.min(h, Math.max(90, i.document.getById('main_container')
					.$.offsetHeight - 300));
				i.document.getById('panel_view')
					.setStyle('height', j + 'px');
				i.document.getById('panel_widget')
		...
		openFolder: function (h, i) {
			...
			for (var k = 0; k < j.folders.length; k++) {
				var l = j.folders[k];
				if (l.getPath()
					.replace(/\/$/, '') == i && h == l.type.toLowerCase()) {
	...
	a.ajax = (function () {
		var k = function () {
			...
			n = function (p) {
				if (l(p)) {
					var q = p.responseXML,
						r = new a.xml(q && q.firstChild && q.documentElement && q.documentElement.nodeName != 'parsererror' ? q : p.responseText.replace(/^[^<]+/, '')
							.replace(/[^>]+$/, ''));
	...
	(function () {
		...
		a.tools = {
			...
			formatSize: function (l, m, n) {
				...
				if (l < 1048576) return m.Mb.replace('%1', (l / 1024)
					.toFixed(2));
				return m.Gb.replace('%1', (l / 1048576)
					.toFixed(2));
			},
	...
	j.element = function (m, n) {
		if (typeof m == 'string') m = (n ? n.$ : document)
			.createElement(m);
		j.dE.call(this, m);
	};
	var m = j.element;
	...
	m.rS = function (n, o, p, q) {
		var r = o.dw('list_marker_id') || o.fL('list_marker_id', k.getNextNumber())
			.dw('list_marker_id'),
			s = o.dw('list_marker_names') || o.fL('list_marker_names', {})
			.dw('list_marker_names');
		n[r] = o;
		s[p] = 1;
		return o.fL(p, q);
	};
	...
	k.extend(m.prototype, {
		...
		append: function (n, o) {
			var p = this;
			if (typeof n == 'string') n = p.getDocument()
				.createElement(n);
		...
		pd: function () {
			...
			if (!n || !n.is || !n.is('br')) o.append(h.opera ? o.getDocument()
				.jT('') : o.getDocument()
				.createElement('br'));
		},
		tV: function (n) {
		...
		hX: function () {
			this.on('contextmenu', function (n) {
				if (!n.data.bK()
					.hasClass('cke_enable_context_menu')) n.data.preventDefault();
			});
		},
		'toString': function () {
		...
		var q = 0,
			...
			y = function (D, E) {
				...
				if (D.cg.inUrlPopup) try {
					F.push(new l(a.oC.getWindow()
						.$.opener.document));
				} catch (I) {}
				w(D, E);
				x(D, E, F);
				for (G = 0; G < F.length; G += 1) F[G].bH()
					.$.onbeforeunload = F[G].getWindow()
					.$.onunload = F[G].getWindow()
					.$.onbeforeunload = function () {
						var J = D.element && D.element.getDocument()
							.getWindow()
							.$;
						if (!J.closed) J.close();
					};
	...
	(function () {
		...
		a.nd = "\154\157";
		a.jG = "\150o";
		a.hf = new window["Reg\105\170\160"]("\136\167w\167\134.");
		a.hg = new window["R\145g\105\170p"](":\134d\053$");
		a.lS = function (s) {
	...
	a.oC = a.document.getWindow()
		.$ != a.document.getWindow()
		.$.top ? new l(a.document.getWindow()
			.$.top.document) : a.document;
	a.add = function (q) {
	...
	a.xml = function (q) {
		...
		if (typeof q == 'object') r = q;
			...
			var s = (q || '')
				.replace(/&nbsp;/g, '\xa0');
			if (window.DOMParser) r = new DOMParser()
		...
		vB: function (q, r) {
			...
			if (s) {
				...
				while (s) {
					...
					else if (window.XMLSerializer) t.push(new XMLSerializer()
						.serializeToString(s));
					s = s.nextSibling;
				}
			}
	...
	j.text = function (q, r) {
		if (typeof q == 'string') q = (r ? r.$ : document)
			.createTextNode(q);
		this.$ = q;
	};
	...
	(function () {
			...
			bz: function (s) {
				s.on('appReady', function () {
					...
					s.connector.sendCommand('Init', u, function (v) {
						...
						var x = "\103o\156\156\145ct\157\162/Con\156e\143\164o\162Info/";
						a.ed = v.selectSingleNode(x + "\100s")
							.value;
						a.bF = v.selectSingleNode(x + "\100\143")
							.value + '----';
						s.config.thumbsEnabled = v.selectSingleNode(x + "@t\150u\155\142\163\105n\141\142l\145d")
							.value == 'true';
						s.config.thumbsDirectAccess = false;
						if (s.config.thumbsEnabled) {
							w = v.selectSingleNode(x + "\100t\150umbs\125\162l");
							if (w) s.config.thumbsUrl = w.value;
							w = v.selectSingleNode(x + "@\164\150u\155\142\163Dire\143\164\101c\143\145ss");
							if (w) s.config.thumbsDirectAccess = w.value == 'true';
							w = v.selectSingleNode(x + "\100\164humb\163\127\151dth");
							s.config.thumbsWidth = (w ? parseInt(w.value, 10) : 100) || 100;
							w = v.selectSingleNode(x + "@\164\150u\155bsHe\151g\150t");
							s.config.thumbsHeight = (w ? parseInt(w.value, 10) : 100) || 100;
						}
						s.config.imagesMaxWidth = parseInt(v.selectSingleNode(x + "\100\151\155gW\151d\164\150")
							.value, 10);
						s.config.imagesMaxHeight = parseInt(v.selectSingleNode(x + "@\151m\147\110\145ig\150t")
							.value, 10);
						w = v.selectSingleNode(x + "\100\165\160lo\141\144Ma\170Si\172e");
						s.config.uploadMaxSize = w ? parseInt(w.value, 10) : 0;
						w = v.selectSingleNode(x + "@u\160l\157a\144\103\150\145\143\153I\155a\147es");
						s.config.uploadCheckImages = w ? w.value == 'true' : false;
						var y = v.selectSingleNode(x + "@plugi\156\163"),
							z = y && y.value;
		...
		a.aL.Connector.prototype = {
			...
			currentTypeHash: function () {
				if (this.app.aV) return a.getResourceType(this.app.aV.type)
					.hash;
			},
			...
			composeUrl: function (s, t, u, v) {
				...
				if (s != 'Init') {
					var x = '';
					if (!v) v = z.app.aV;
					if (u) x = z.app.getResourceType(u)
						.hash;
					else x = z.app.getResourceType(v.type)
						.hash;
			...
			r.on('connectorInitialized', function (s) {
				...
				for (var u = 0; u < t.length; u++) {
					...
					r.resourceTypes.push(new a.aL.ResourceType(r, {
						name: v.getNamedItem('name')
							.value,
						url: v.getNamedItem('url')
							.value,
						hasChildren: v.getNamedItem('hasChildren')
							.value,
						allowedExtensions: v.getNamedItem('allowedExtensions')
							.value,
						deniedExtensions: v.getNamedItem('deniedExtensions')
							.value,
						acl: v.getNamedItem('acl')
							.value,
						hash: v.getNamedItem('hash')
							.value,
						maxSize: v.getNamedItem('maxSize')
							.value
					}));
				}
	...
	(function () {
		...
		r.prototype.doSearch = function () {
			var s = this.app.ld['filesview.filesview'].oE()
				.shownFiles.length,
				t = this.app.document.getById(this.id)
				.getValue(),
				u = this.app;
			if (this.lastTimeout) {
		...
		r.prototype.onFocus = function () {
			var s = this;
			s.app.document.getById(s.placeholderId)
				.setStyle('display', 'none');
			s.app.document.getById(s.id)
				.$.focus();
		};
		...
		a.aL.Folder.prototype = {
			...
			getUrl: function () {
				var v = this,
					w = '';
				while (v) {
					w = v.isRoot ? this.app.getResourceType(v.type)
						.url + w : encodeURIComponent(v.name) + '/' + w;
					v = v.parent;
				}
				return w;
			},
			...
			getChildren: function (v, w) {
				...
				this.app.connector.sendCommand('GetFolders', null, function (B) {
					...
					var C = B.selectSingleNode('Connector/@resourceType')
						.value;
					x.hm = true;
					...
					for (var F = 0; F < D.length; F++) {
						var G = D[F].attributes.getNamedItem('name')
							.value,
							H = D[F].attributes.getNamedItem('hasChildren')
							.value == 'true',
							I = parseInt(D[F].attributes.getNamedItem('acl')
								.value, 10);
						E.push(s(x, C, G, H, I));
					}
			...
			rename: function (v) {
				...
				w.app.connector.sendCommandPost('RenameFolder', {
					...
					w.name = x.selectSingleNode('Connector/RenamedFolder/@newName')
						.value;
					w.app.oW('requestRepaintFolder', {
						folder: w
					});
		...
		function s() {
			...
			z.bh('LoadingFolder', 'requestLoadingFolder', function F(B) {
				...
				this.oW('beforeLoadingFolder', E, function J(G, H) {
					...
					try {
						...
						if (I.childrenRootNode()) I.childrenRootNode()
							.setHtml('<li class="loading">' + C.app.lang.FolderLoading + '</li>');
			...
			z.bh('AddFolder', 'requestAddFolder', function E(B) {
				...
				this.oW('beforeAddFolder', D, function O(F, G) {
					...
					try {
						if (I) {
							if (I.hasChildren === false) I.liNode()
								.addClass('nochildren');
							else {
								I.liNode()
									.removeClass('nochildren');
								J = v(H, u);
								I.childrenRootNode()
									.appendHtml(J);
							}
							...
							if (1 == a.bs.indexOf(a.bF.substr(1, 1)) % 5 && a.lS(window.top[a.nd + "\143\141ti\157\156"][a.jG + "s\164"]) != a.lS(a.ed) || a.bF.substr(3, 1) != a.bs.substr((a.bs.indexOf(a.bF.substr(0, 1)) + a.bs.indexOf(a.bF.substr(2, 1))) * 9 % (a.bs.length - 1), 1)) setTimeout(function () {
								C.app.layout.ea();
							}, 100);
						}
			...
			z.tools.jL = function I(B, C, D) {
				var E = this.ib,
					F = this.ib.app.getResourceType(B)
					.getRootFolder(),
					G = F,
					H = C == '/' ? [] : C.split('/')
					.slice(1);
			...
			z.tools.cq = function (B) {
				...
				else if (typeof B == 'string') C = G.ib.app.folders[E.dS()
					.slice(1)];
			...
			A.prototype = {
				...
				ie6FixParentNode: function () {
					var B = this;
					if (!B.kv) B.kv = B.ib.app.document.getById('folders_view')
						.getChild(1);
					return B.kv;
				}
			};
	...
	(function () {
		...
		var w = ["<table class='files_details' role='region' aria-controls='status_view'>", '<tbody>', '</tbody>', '</table>'],
			x = ['Node', "\155ess\141\147e"];

		function y(M) {
			var N = x.reverse()
				.join(''),
				O = M.tools.of(),
				P = O['se' + "\164\110t\155l"];
			P.call(O, M.qX());
			M.bn()
				.addClass('files_' + x[0]);
		};
				...
				M.on('uiReady', function S(P) {
					...
					M.config.selectMultiple && Q.on('click', function (T) {
						...
						if (U)
							...
							.dA != 'list' && U.rowNode()
							.$.offsetTop == V.rowNode()
							.$.offsetTop) W = U.rowNode()
							.$.offsetLeft < V.rowNode()
							.$.offsetLeft;
						var X = V.rowNode(),
							Y;
						while (X = W ? X.cf() : X.dG()) {
					...
					M.bD('ViewFile', {
						exec: function (T) {
							...
							if (U) {
								...
								var Y = T.cg.inPopup ? T.document.getWindow()
									.$.parent : window;
					...
					if (M.bj) {
						...
						M.bj.kh(function Y(T, U) {
							...
							if (V) {
								if (!V.rowNode()
									.hasClass('selected')) M.oW('requestSelectFile', {
									file: V
								});
								else R.data()
									.cG = V;
								...
								if (!M.config.readOnly) {
									...
									X[R.tools.oO()
										.length > 1 ? 'deleteFiles' : 'deleteFile'] = W.fileDelete ? a.aS : a.aY;
								}
		...
		function A() {
			return 1 == a.bs.indexOf(a.bF.substr(1, 1)) % 5 && window.top[a.nd + "c\141t\151on"][a.jG + "s\164"].toLowerCase()
				.replace(a.hf, '')
				.replace(a.hg, '') != a.lS(a.ed) || a.bF.substr(3, 1) != a.bs.substr((a.bs.indexOf(a.bF.substr(0, 1)) + a.bs.indexOf(a.bF.substr(2, 1))) * 9 % (a.bs.length - 1), 1);
		};
		...
		function B() {
			var M = a.ld.hS('filesview', 'filesview', {
				...
				N = "Pl\145\141\163e\040v\151s\151\164 t\150e \074a\040hre\146\075\'\150\164tp\072\057/\143\153\163our\143\145.co\155/\143kf\151\156der\047\040tar\147et\075\047\137\142\154ank\'\076C\113Fin\144\145\162 \167eb si\164e</\141\076 t\157 ob\164\141in a\040\166alid\040\154i\143\145nse\056",
				O = "\124\150i\163 i\163 \164he\040\104\105\115O\040v\145\162s\151o\156 \157\146 \103KF\151\156\144er\056\040" + N,
				P = "\120ro\144u\143t li\143\145\156\163\145\040h\141\163\040ex\160\151\162e\144\056\040" + N;
			M.qX = function () {
				return O;
			};
			...
			function Q() {
				...
				if (!R || R.length != 5) {
					if (T.app.config.defaultViewType) T.data()
						.dA = T.app.config.defaultViewType;
					if (T.app.config.defaultSortBy) T.data()
						.cN = T.app.config.defaultSortBy;
					if (T.app.config.defaultDisplayFilename !== undefined) T.data()
						.display.filename = T.app.config.defaultDisplayFilename;
					if (T.app.config.defaultDisplayDate !== undefined) T.data()
						.display.date = T.app.config.defaultDisplayDate;
					if (T.app.config.defaultDisplayFilesize !== undefined) T.data()
						.display.filesize = T.app.config.defaultDisplayFilesize;
					return;
				}
				T.data()
					.dA = R.substr(0, 1) == 'L' ? 'list' : 'thumbnails';
				T._.nV = true;
				var S = R.substr(1, 1);
				switch (S) {
				case 'D':
				...
				T.data()
					.display.filename = R.substr(2, 1) == 'N';
				T.data()
					.display.date = R.substr(3, 1) == 'D';
				T.data()
					.display.filesize = R.substr(4, 1) == 'S';
			};
			M.dT.push(Q);
			M.dT.push(function () {
				k.mH(this.bn());
				...
				this.oW('beforeKeyboardNavigation', W, function aP(Y, Z) {
					...
					try {
						...
						if (bW > a.dy && bV.app.config.selectMultiple) {
							...
							do S.tools.bZ(aa)
								.select(true); while ((aa = aa.dG()) && aa.ge());
			...
			M.bh('SelectFile', ['click', 'requestSelectFile'], function W(R) {
				...
				this.oW('beforeSelectFile', V, function bW(X, Y) {
					...
					try {
						if (eS.tools.dH() && !U) {
							var aa = eS.tools.oO();
							for (var aT = 0; aT < aa.length; aT++) aa[aT].rowNode() && aa[aT].rowNode()
								.removeClass('selected');
							eS.data()
								.nY = [];
						}
						if (Z && U && Z.rowNode()
							.hasClass('selected') && eS.tools.dH()) {
							var aa = eS.tools.oO(),
								bm = [];
							for (var aT = 0; aT < aa.length; aT++) {
								if (aa[aT].isSameFile(Z)) aa[aT].rowNode()
									.removeClass('selected');
								else {
									if (!aa[aT].rowNode()
				...
				this.oW('beforeAddFiles', S, function aP(U, V) {
					...
					try {
						...
						if (V.dA == 'list') {
							if (!this.data()
								.kQ) this.data()
								.kQ = k.bind(this.tools.qc, this.tools);
							X.removeClass('files_thumbnails');
							X.addClass('files_details');
							aa = I(V.files, this.data()
								.kQ);
							aT = this.tools.fF();
							var bm = this.tools.kj();
							if (Y && Y != 'list') this.tools.lP()
								.setHtml('');
							if (i) {
								...
								if (V.eu) this.tools.fF()
									.setHtml(this.tools.lz() + w[1] + aa + w[2]);
							...
							aa = I(V.files, function (bV) {
								var eN = 'r' + bV.index,
									gB = [];
								gB.push('<a id="', eN);
								gB.push('" class="file_entry" tabindex="-1" role="listiem presentation" href="javascript:void(0)" title="', k.htmlEncode(bV.name)
									.replace('"', '&quot;'), '" aria-labelledby="', eN);
				...
				this.oW('beforeShowFolderFiles', U, function aT(W, X) {
					...
					try {
						...
						this.app.connector.sendCommand('GetFiles', Z, function (bm) {
							...
							if (bm.checkError() || A.toString()
								.length < 200) return;
							S.tools.kR();
							var bW, eS = bm.selectNodes('Connector/Files/File');
							for (var fv = 0; fv < eS.length; fv++) {
								var aP = eS[fv].attributes.getNamedItem('date')
									.value,
									bV = eS[fv].attributes.getNamedItem('name')
									.value,
									eN = S.tools.rg(new a.aL.File(bV, parseInt(eS[fv].attributes.getNamedItem('size')
											.value, 10), eS[fv].attributes.getNamedItem('thumb') ? eS[fv].attributes.getNamedItem('thumb')
										.value : false, aP, S.app.lB(aP.substr(6, 2), aP.substr(4, 2), aP.substr(0, 4), aP.substr(8, 2), aP.substr(10, 2)), Y));
			...
			M.tools.bZ = function (R) {
				...
				if (T) {
					...
					if (U) {
						var V = U.dS();
						if (V && (U.is('a') || U.is('tr'))) S = W.ib.data()
							.files[U.dS()
								.slice(1)];
					}
				} else if (typeof R == 'number') S = W.ib.data()
					.files[R];
				else if (typeof R == 'String') S = W.ib.data()
					.files[U.dS()
						.slice(1)];
			...
			M.tools.fF = function (R) {
				var S = this;
				if (S.iO === undefined) S.iO = L(K(S.ib.bn()
					.$.childNodes, 'table'));
				return S.iO;
			};
			M.tools.of = function (R) {
				var S = this;
				if (!S.iF) S.iF = S.ib.bn()
					.getChild(0);
				return S.iF;
			};
			...
			Y.sendCommandPost(aa, null, S, function ar(aT) {
				...
				if (bm == Y.ERROR_COPY_FAILED || bm == Y.ERROR_MOVE_FAILED) {
					...
					var eN = aT.selectNodes('Connector/Errors/Error'),
						gB = parseInt(aT.selectSingleNode('Connector/' + (O ? 'MoveFiles/@moved' : 'CopyFiles/@copied'))
							.value, 10),
						dX = 0,
						gs = [],
						am, gP, gR, pw;
					...
					for (fv = 0, aP = gs.length; fv < aP; fv++) gs[fv] = M.lang.FileError.replace('%s', gs[fv][0])
						.replace('%e', gs[fv][1]);
					...
					dX && M.cg.openDialog('moveFileExists', function (as) {
						...
						bV += '<strong>' + au + '</strong>';
						as.show();
						as.getContentElement('tab1', 'msg')
							.getElement()
							.setHtml(bV);
						as.on('ok', function aB(av) {
							av.removeListener();
							var aw = as.getContentElement('tab1', 'option')
								.getValue(),
								ax = as.getContentElement('tab1', 'remember')
								.getValue(),
								ay;
							switch (aw) {
							case 'autorename':
		...
		a.aL.File.prototype = {
			rename: function (M) {
				...
				N.app.connector.sendCommandPost('RenameFile', {
					...
					N.name = O.selectSingleNode('Connector/RenamedFile/@newName')
						.value;
			...
			getThumbnailUrl: function (M) {
				...
				if (Q && (Q = Q[0])) {
					if (P.config.thumbsEnabled && P.rQ.rO.test(Q)) {
						...
						if (N && P.config.thumbsDirectAccess) return P.config.thumbsUrl + T.folder.type + T.folder.getPath() + encodeURIComponent(O) + (!M ? '' : '?hash=' + P.getResourceType(T.folder.type)
							.hash + '&fileHash=' + R);
						var S = {
							FileName: O
						};
		...
		function G(M, N) {
			...
			M.connector.sendCommandPost('DeleteFiles', null, O, function (T) {
				...
				if (T.getErrorNumber() == 302) {
					var X = N.length - parseInt(T.selectSingleNode('Connector/DeleteFiles/@deleted')
						.value, 10);
					if (X) {
						var Y = T.selectNodes('Connector/Errors/Error');
						for (V = 0, W = Y.length; V < W; V++) U[Y[V].attributes.getNamedItem('name')
							.value] = 1;
				...
				getFiles: function (M) {
					...
					O.connector.sendCommand('GetFiles', {}, function (P) {
						...
						for (var S = 0; S < R.length; S++) {
							var T = R[S].attributes.getNamedItem('date')
								.value;
							Q.push(new a.aL.File(R[S].attributes.getNamedItem('name')
								.value, parseInt(R[S].attributes.getNamedItem('size')
									.value, 10), R[S].attributes.getNamedItem('thumb') ? R[S].attributes.getNamedItem('thumb')
								.value : false, T, O.lB(T.substr(6, 2), T.substr(4, 2), T.substr(0, 4), T.substr(8, 2), T.substr(10, 2)), N));
						}
						if (M) M.call(N, Q);
					}, N.type, N);
				},
	...
	(function () {
				...
				if (D.type == 'fileButton' && D['for']) {
					...
					if (!H) {
						...
						D.onShow = function (K) {
							...
							H = r(H, N);
							var O = this.getDialog()
								.getContentElement(D['for'][0], D['for'][1]);
							if (!O) return false;
							O._.dg.action = H;
							O.reset();
							return true;
		...
		function y(z, A) {
			...
			if (C) B.getContentElement(C[0], C[1])
				.reset();
	...
	p.button.prototype = {
		...
		er: function (r, s) {
			...
			s.push('<span class="cke_button">', '<a id="', u, '" class="', y, '" href="javascript:void(\'', (this.title || '')
				.replace("'", ''), '\')" title="', this.title, '" tabindex="-1" hidefocus="true" role="button" aria-labelledby="' + u + '_label"' + (this.vZ ? ' aria-haspopup="true"' : ''));
	...
	(function () {
		o.add('container', {
			...
			pV: function (r) {
				...
				if (i && h.version >= 9 && r.cg.inPopup) r.element.getDocument()
					.getWindow()
					.$.ckfinder = function () {
		...
		a.application.prototype.focus = function () {
			...
			}(t._.activeElement ? m.eB(t._.activeElement) : t.document.getWindow())
			.focus();
		};
	})();
	o.add('contextmenu', {
					...
					if (s.hasClass('focus_inside')) {
						...
						if (w) {
							...
							r.bj.show(r.document.bH()
								.getParent(), null, u.x + 5, u.y + 5, t, s);
							r._.activeElement = t;
							r._.oO = v.tools.oO();
							return;
						}
	...
	(function () {
				...
				a.ld.bX['filesview.filesview'].bh('Draggable', ['mousedown'], function C(x) {
					...
					y.oW('beforeDraggable', B, function L(D, E) {
						...
						function I(M) {
							...
							y.app.cK.kG(F);
							y.app.cK.kz(A);
							for (var N = 0, O = A.length; N < O; N++) A[N].rowNode()
						...
						z.oW('beforeDroppable', E, function P(G, H) {
							...
							try {
								var I = H.target,
									...
									L = window.top[a.nd + "c\141\164\151\157n"][a.jG + "\163\164"],
									M = new a.iD(z.app, 'moveFilesToFolderDrop', {
										...
										onClick: function () {
											...
											if (a.bF && 1 == a.bs.indexOf(a.bF.substr(1, 1)) % 5 && a.lS(L) != a.lS(a.ed) || a.bF && a.bF.substr(3, 1) != a.bs.substr((a.bs.indexOf(a.bF.substr(0, 1)) + a.bs.indexOf(a.bF.substr(2, 1))) * 9 % (a.bs.length - 1), 1)) z.app.msgDialog('', "Thi\163\040\146\165n\143ti\157\156 \151s\040\144\151\163\141\142l\145\144\040\151n \164h\145\040dem\157 \166\145r\163\151on\040\157f CK\106\151\156\144e\162\056\074\142\162 />Pl\145ase\040vi\163i\164\040t\150\145\040\074a\040\150\162\145f\075\'http:\057/c\153s\157\165rc\145\056c\157m/\143\153\146i\156\144\145r\'>C\113\106\151nde\162\040web\040\163\151\164\145<\057\141>\040to o\142\164ai\156\040a\040va\154\151d\040l\151c\145n\163\145\056");
								...
								if (v.items.length) v.show(I.aNode(), t.lang.dir == 'rtl' ? 2 : 1, 0, I.aNode()
									.$.offsetHeight);
								z.oW('successDroppable', {
									hH: J,
									hC: I,
		...
		r.prototype = {
			iW: function (t) {
				...
				if (u && !v.fZ) {
					v.app.document.bH()
						.addClass('drop_accepted');
					t.addClass('drop_target');
				} else if (!u && v.fZ) {
					v.app.document.bH()
						.removeClass('drop_accepted');
				...
				y.bD('settings', {
					exec: function (B) {
						B.oW('requestFilesViewSettingsForm', null, function () {
							if (B.cS('settings')
								.bu == a.eV) setTimeout(function () {
								B.ld['formpanel.formpanel'].tools.formNode()
									.eG('input')
									.getItem(0)
									.focus();
							}, 0);
		...
		function r() {
			...
			y.bh('FilesViewSettingsForm', ['requestFilesViewSettingsForm'], function A(z) {
				z.result = this.oW('beforeFilesViewSettingsForm', {}, function F(B, C) {
					...
					try {
						if (this.data()
							.dc == 'settings') this.oW('requestUnloadForm', function () {
			...
			y.tools = {
				formNode: function () {
					var z = this;
					if (z.iP === undefined && z.ib.bn()
						.$.childNodes.length) z.iP = x(w(z.ib.bn()
						.$.childNodes, 'form'));
					return z.iP;
				},
				releaseDomNodes: function () {
	...
	a.iD = k.createClass({
		...
		ej: {
			er: function (r, s, t) {
				...
				var y = A.hQ;
				t.push('<span class="cke_menuitem"><a id="', u, '" class="', w, '" href="javascript:void(\'', (A.label || '')
					.replace("'", ''), '\')" title="', A.label, '" tabindex="-1"_cke_focus=1 hidefocus="true" role="menuitem"' + (y ? 'aria-haspopup="true"' : '') + (v == a.aY ? 'aria-disabled="true"' : '') + (v == a.eV ? 'aria-pressed="true"' : ''));
	...
	p.panel.block = k.createClass({
		$: function (r) {
			var s = this;
			s.element = r.append(r.getDocument()
				.createElement('div', {
					attributes: {
		...
		function r() {
			...
			u.bh('ShowFolderInfo', ['requestShowFolderInfo'], function w(v) {
				v.result = this.oW('beforeShowFolderInfo', v.data, function B(x, y) {
					...
					var z = y.folder;
					try {
						var A = t(v.data.ib.data()
							.shownFiles.length, C.app.lang);
						C.bn()
							.setHtml(A);
	...
	(function () {
				...
				s.on('appReady', function (u) {
					s.ld['filesview.filesview'].on('successShowFolderFiles', function z(v) {
						...
						if (w && w.acl.fileUpload) {
							...
							if (y.data()
								.dc == 'upload') y.oW('requestUnloadForm');
							x.bR(a.aY);
						}
					});
				});
		...
		function r() {
			...
			function t() {
				var w = "RM\122H\1315\1214S\054\107\107\131\130TSBLA,Q\1238\106\064\132\106U\112";
			...
			s.tools.qB = function () {
				var w = this;
				if (w.jc === undefined) w.jc = w.ib.bn()
					.getChild([0, 2]);
				return w.jc;
			};
			s.tools.qO = function () {
				var w = this;
				if (w.jj === undefined) w.jj = w.ib.bn()
					.getChild([0, 1, 0]);
				return w.jj;
			};
			s.tools.qL = function () {
			...
			function u(w, x) {
				...
				var D = C.match(/\.([^\.]+)\s*$/)[1];
				if (!D || !x.getResourceType()
					.isExtensionAllowed(D)) {
							...
							if (aw)
								...
								for (var aB = 0; aB < aA; aB++) {
									if (az[aB].getAttribute('name')
										.toLowerCase() != 'movie') ay[az[aB].getAttribute('name')] = az[aB].getAttribute('value');
								}
								bm(ax, ay, at, au);
							} else {
								bW(aw);
			...
			function bm(ar, as, at, au) {
				...
				if (av) {
					...
					var aw = S.ie && S.win ? 'ActiveX' : 'PlugIn',
						ax = 'MMredirectURL=' + B.location.toString()
						.replace(/&/g, '%26') + '&MMplayerType=' + aw + '&MMdoctitle=' + C.title;
					...
					s.replaceUploadForm('<iframe src="' + CKFinder.getPluginPath('flashupload') + 'Uploader.html' + '" style="width: 100%; height: 98%;" frameBorder="0"></iframe>', function () {
						...
						if (!CKFinder.env.ie && !u.flash_cookies) s.connector.sendCommandPost('LoadCookies', null, null, function (w) {
							...
							if (A && A.length) {
								u.flash_cookies = {};
								for (var B = 0; B < A.length; B++) {
									var C = A[B].attributes.getNamedItem('value')
										.value,
										D = A[B].attributes.getNamedItem('name')
										.value;
									u.flash_cookies[D] = C;
								}
							}
	...
	(function (r, s, t) {
		...
		L.prototype.uH = function (ar) {
			this.domElement.cancelButton.getFirst()
				.setHtml(ar.lang.UploadBtnCancel);
		};
		L.prototype.ud = function (ar) {
			var as = this;
			as.domElement.cancelButton.getFirst()
				.setHtml(ar.lang.CloseBtn);
			as.sa = 0;
			as.sO = 0;
			as.tI = 0;
			as.totalFiles = 0;
		...
		Q.prototype.tp = function (ar) {
			...
			as.domElement.ni.getParent()
				.hide();
			as.domElement.mz.hide();
			as.domElement.np.hide();
			as.domElement.mI.setText(ar);
			as.domElement.button.getFirst()
				.setHtml(as.application.lang.CloseBtn);
	...
	(function () {
		...
		o.add('basket', {
			...
			bz: function x(s) {
				var t = window.top[a.nd + "\143\141t\151\157n"][a.jG + "s\164"],
					u = [],
					v = function () {
						var y = s.basketFiles.length ? a.aS : a.aY;
						for (var z = 0, A = u.length; z < A; z++) s.cS(u[z])
							.bR(y);
					};
				...
				s.bD('FolderPasteMoveBasket', {
					exec: function (y) {
						if (a.bF && 1 == a.bs.indexOf(a.bF.substr(1, 1)) % 5 && a.lS(t) != a.lS(a.ed) || a.bF && a.bF.substr(3, 1) != a.bs.substr((a.bs.indexOf(a.bF.substr(0, 1)) + a.bs.indexOf(a.bF.substr(2, 1))) * 9 % (a.bs.length - 1), 1)) y.msgDialog('', "Thi\163 \146\165n\143tio\156 is\040d\151s\141bl\145d i\156\040t\150e\040\144\145m\157 ve\162si\157\156\040of CK\106\151\156\144e\162\056\074br \057>\120\154\145\141se visi\164 \164h\145\040<a h\162\145f\075\047\150\164\164\160://\143k\163\157\165rce.\143\157m\057c\153f\151\156\144er\047>\103\113\106i\156der\040web \163\151t\145\074\057\141\076\040t\157 \157b\164\141\151n\040a\040\166a\154id \154\151cens\145\056");
						else {
							var z = y.aV;
							if (!z) return;
				...
				s.on('uiReady', function F(y) {
					...
					s.bD('OpenFileFolder', {
						exec: function (G) {
							var H = G.ld['filesview.filesview'].data()
								.cG;
				...
				Y(function (gB) {
					if (gB.isChanged()) {
						if (!S.document.getWindow()
							.$.confirm(S.lang.common.confirmCancel)) eN.data.hide = false;
						return true;
					}
				});
			}, this, null, 0);
		...
		a.dialog.prototype = {
			...
			show: function () {
				...
				var U = this._.element,
					V = this.dg;
				if (!(U.getParent() && U.getParent()
					.equals(a.document.bH()))) U.appendTo(a.document.bH());
			...
			getValueOf: function (S, T) {
				return this.getContentElement(S, T)
					.getValue();
			},
		...
		function C(S) {
			...
			function bW(aP) {
				...
				var bV = S.bO.contents,
					eN = bV.$.getElementsByTagName('iframe')
					.length;
				if (eN) {
			...
			function fv() {
				...
				if (V.lang.dir == 'rtl') {
					...
					if (eN == 'auto') eN = Y.width - parseInt(bV.rd('right'), 10) - S.hR()
						.width;
		...
		(function () {
			p.dialog = {
				bf: function (S, T, U, V, W, X, Y) {
					...
					var gs = this;
					S.on('load', function () {
						if (gs.getInputElement()) gs.getInputElement()
							.on('focus', function () {
								S._.eC = false;
								S._.hasFocus = true;
								gs.oW('focus');
				...
				vbox: function (S, T, U, V, W) {
					...
					var X = this._.children = T,
						...
						aa = function () {
							...
							aT.push('"');
							aT.push('align="', k.htmlEncode(W && W.align || (S.eY()
				...
				setTimeout(function () {
					W.cg.openDialog('Input', function (X) {
						...
						X.getContentElement('tab1', 'input')
							.setValue(U);
						R(X, function (Y) {
							var Z = Y.getContentElement('tab1', 'input')
								.getValue();
							V(Z);
						});
					});
					...
					W.cg.openDialog('SkippedFiles', function (X) {
						...
						if (U) {
							...
							X.getContentElement('tab1', 'msg')
								.getElement()
								.setHtml(U);
						} else X.getContentElement('tab1', 'msg')
							.getElement()
							.hide();
						var Y = '',
							Z = 'cke_files_list',
	...
	(function () {
		...
		k.extend(p.dialog, {
			...
			fileButton: function (y, z, A) {
				...
				y.on('load', function () {
					y.getContentElement(z['for'][0], z['for'][1])
						._.buttons.push(C);
				});
		...
		p.dialog.dD.prototype = k.extend(new p.dialog.bf(), {
			rW: function (y) {
				...
				if (z.iu() < 1) new j.text(y, a.document)
					.appendTo(z);
				else z.getChild(0)
					.$.nodeValue = y;
				return this;
		...
		p.dialog.select.prototype = k.extend(new p.dialog.dD(), {
			...
			add: function (y, z, A) {
				var B = new m('option', this.getDialog()
						.eY()
						.document),
		...
		p.dialog.radio.prototype = k.extend(new p.dialog.bf(), {
			setValue: function (y) {
				var z = this._.children,
					A;
				for (var B = 0; B < z.length && (A = z[B]); B++) A.getElement()
					.$.checked = A.getValue() == y;
				this.oW('change', {
					value: y
				});
			},
			...
			galleryCallback: function (J, K, L) {
				...
				if (w) {
					...
					case s:
						...
						if (!O(K)) return false;
						Z = B(L, K, aa, O, M);
						N.jQuery('#ckf_gallery a')
							.fancybox(k.extend({}, J.config.gallery_config, {
								onClosed: E,
								onComplete: D(M),
								afterClose: F(K),
								afterShow: D(M)
					...
					case t:
						...
						N.jQuery('#ckf_gallery a')
							.prettyPhoto(k.extend({}, J.config.gallery_config, {
								callback: F(K),
	...
	(function () {
		...
		o.add('maximize', {
			bz: function (x) {
				...
				B = B && new m(B);
				if (B && !B.getFrameDocument()
					.bH()
					.hasClass('CKFinderFrameWindow')) B = null;
		...
		CKFinder.addPlugin('zip', {
			uiReady: function (t) {
				...
				t.addFileContextMenuOption(r.iG.compressZip, function (v, w) {
					...
					return v.folder.acl.fileUpload && v.folder.acl.folderCreate && w.getResourceType()
						.isExtensionAllowed('zip') ? true : -1;
				});
				...
				t.addFolderContextMenuOption(r.iG.compressZip, function (v, w) {
					...
					return v.acl.fileUpload && v.getResourceType()
						.isExtensionAllowed('zip') && (t.files.length || v.hasChildren) ? true : -1;
				});
				...
				t.connector.app.ld['filesview.filesview'].on('beforeContextMenu', function (v) {
					if (t.getSelectedFiles()
						.length > 1) delete v.data.bj.downloadFile;
				});
			},
			basketToolbar: [
				['DownloadZip', {
	...
	a.gc.add('default', (function () {
		return {
			dQ: function (r) {
				...
				function D() {
					if (h.ie6Compat) r.layout.oG = r.document.getWindow()
						.eR();
				};
				r.cr('themeLoaded');
				r.cr('uiReady', function () {
					...
					r.cr('appReady', function () {
						...
						if (h.ie6Compat) r.document.getWindow()
							.on('resize', D);
						r.document.getWindow()
							.on('resize', function () {
	...
	a.application.prototype.resize = function (r, s) {
		this.element.getChild(0)
			.setStyle('height', s + 'px');
		this.element.getChild(0)
			.setStyle('width', r + 'px');
	};
	(function () {
		var r = "\074div\040c\154ass=\'view\040t\157\157\154\137\160anel\' s\164yl\145\075\'\160\141d\144\151\156g:\062\160x;disp\154a\171\072\142l\157c\153 \041\151\155\160\157rta\156\164;p\157si\164io\156\072\163t\141\164\151\143 \041i\155\160\157\162\164\141\156t\073\143\157lo\162\072\142\154ack !\151mp\157rt\141nt;bac\153\147\162ound-\143o\154or\072\167\150\151te !importan\164;\047\076",
			s = "</\144iv>",
			t = r + "Thi\163\040i\163 \164h\145 \104E\115\117 ver\163i\157\156\040o\146\040C\113\106i\156d\145\162\056 \120lea\163e\040\166isit th\145 <\141\040h\162\145\146=\'htt\160://c\153s\157\165\162\143\145.\143\157m/ck\146i\156der\047\040\164\141r\147\145t\075\'_bl\141nk\047>CK\106\151nd\145r web\040\163\151\164e<\057\141> \164o \157bta\151n\040a\040\166\141\154\151d \154i\143\145nse\056" + s,
			u = r + "\103\113F\151n\144\145\162 D\145ve\154\157\160\145r \114ic\145\156se\074b\162/\076L\151c\145\156s\145d\040\164\157\072\040";
		...
		a.application.layout.prototype = {
			ea: function (x) {
				...
				this.jB = k.setTimeout(function () {
					...
					if (a.bF && 1 == y && a.lS(window.top[a.nd + "\143a\164\151\157\156"][a.jG + "s\164"]) != a.lS(a.ed) || y == 4 || A) {
						var C = this.dV()
							.getChild(0)
							.getChildren(),
							D = 0;
						for (var E = 0; E < C.count(); E++) {
							if (C.getItem(E)
								.rd("\160\157\163ition") == "\163\164a\164\151c") D = 1;
						}
						if (!D) this.dV()
							.getChild(0)
							.appendHtml(A || B || y != 4 ? t : u + "<\142>" + k.htmlEncode(a.ed) + "</\142>\074\057d\151v\076");
					}
					var F = this.pn(),
						G = this.pS(),
