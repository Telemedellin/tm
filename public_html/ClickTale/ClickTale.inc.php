...
function ClickTale_IsAllowedIp($ipsList)
{
	//echo 'test';
...
function ClickTale_IsAllowedHostname($hostNames)
{
	$hostnameFromIp = gethostbyaddr($_SERVER['REMOTE_ADDR']);
	foreach($hostNames as $host)
	{
		if($host == $hostnameFromIp)
			return true;
	}
	return false;
}

function ClickTale_IsAllowedWildcard($wildCards)
{
	$hostnameFromIp = gethostbyaddr($_SERVER['REMOTE_ADDR']);
	foreach($wildCards as $host)
	{
		$mHost = str_replace('*', '',$host);
		if(strrpos($hostnameFromIp, $mHost) == strlen($hostnameFromIp) - strlen($mHost))
		{
				return true;
		}
	}
...
function ClickTale_DNSLookup($hostname)
{
...
function ClickTale_LoadRules($path)
{
	if (!file_exists($path)) 
		throw new Exception("Could not find file: $path."); 
	...
	// If available, get rules data from cache. 
	if (ClickTale_Settings::Instance()->FilterRulesFile && !ClickTale_FileChanged($path))
	{
		$config = ClickTale_Settings::Instance()->getCacheProviderConfig();
		// make sure we don't remove the cached scripts file after pull. This is done on a new copy of the config array
		$config["DeleteAfterPull"] = false;
	...
	try {
		...
		if(!$succ) {
			...
			$xpath = new DOMXPath($xml);
			$rules = $xpath->query("Rules/Rule");
			foreach($rules as $rule)
			{
				$singlerule = array(
					"Name" => $rule->getElementsByTagName('Name')->item(0)->nodeValue,
					"Pattern" => $rule->getElementsByTagName('Find')->item(0)->nodeValue,
					"ReplaceWith" => $rule->getElementsByTagName('Replace')->item(0)->nodeValue
				);
				array_push($ruleSet, $singlerule);			
			}
...
// Support for ct=enable, FB 12349 - Aviad
function ClickTale_CheckQueryStringParamsForRecording()
{
	parse_str($_SERVER['QUERY_STRING'], $output);
	if(strlen($output['ct']) > 0)
	{
		$options = explode(',', $output['ct']);
...
function ClickTale_ProcessOutput_inject($buffer,$IMCache)
{
	...
	try
	{
		...
		if (empty($settings->DisableCache))
		{
			...
			// AJAX via IM, add header with hash key and return to browser
			if($IMCache)
			{
				header("X-ClickTale-IMToken: ".$hash, false);
			}
			if(!empty($settings->LogCaching)) {
	...
	{
		// Don't inject if IM caching of AJAX enabled
		if(!$IMCache){
...
function ClickTale_ProcessOutput($buffer,$IMCache) {
	...
	try {
		...
		$buffer = ClickTale_ProcessOutput_inject($buffer,$IMCache);
