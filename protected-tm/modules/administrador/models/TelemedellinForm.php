 ...
 */
class TelemedellinForm extends CFormModel
{
	public $id;
	public $nombre;
	public $texto;
	...
	public function guardar()
	{
		...
		if( $this->validate() )
		{
			...
			else //Nuevo registro
			{
				...
				$pagina->tipo_pagina_id = 2; //Página genérica estándar
			}
			...
			if( isset($this->id) ) //Actualizando
			{
				$pgP = PgGenericaSt::model()->findByAttributes(array('pagina_id' => $pagina_id));
			}
			else //Nuevo registro
			{
				...
				$pgP = new PgGenericaSt;
				$pgP->pagina_id 	= $pagina_id;
			}

			$pgP->texto 		= $this->texto;
	...
	{
		return 'backgrounds/telemedellin/' . date('Y') . '/';		
	}
