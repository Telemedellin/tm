		...
		<div>
			<script>
				function fnSuccess(data){
					jQuery('#resendStatus').html(data);
					setTimeout(function(){ jQuery('#resendStatus').html(""); },3000);
				}
			</script>
...
<?php 
$script = "
	jQuery('ul.auth-item li').each(function(){
		var li = jQuery(this);
		var authitem = li.attr('alt');
		var action = '<?php echo Yii::app()->user->ui->getRbacAjaxAssignmentUrl()?>';
		var actionGetBz = '" . Yii::app()->user->ui->getRbacAjaxGetAssignmentBzUrl(). "';
		var actionSetBz = '" . Yii::app()->user->ui->getRbacAjaxSetAssignmentBzUrl() ."';
		var loadingUrl = '" . Yii::app()->user->ui->getResource('loading.gif')  ."';
		...
		li.find('.grant-icon').click(function(){
			...
			+'\"userid\": \"" . $model->getPrimaryKey(). "\" , \"setflag\": '+setFlag+' }';	
			var loader = _li.find('span.loader');
			
			loader.html('<img src=\"+loadingUrl+\">');
			jQuery('#_errorResult').html('');
			jQuery.ajax({
				url: action,
				type: 'post',
				async: true,
				...
				success: function(data, textStatus, jqXHR){
					...
					if(data.result == true){
						...
						li.find('.grant-icon').attr('src','" . $handoffimgurl. "');
						li.find('.edit-icon').show();
					}else{
						_li.removeClass('checked');
						li.find('.grant-icon').attr('src','" . $handimgurl ."');
						li.find('.edit-icon').hide();
					}
				},
				error: function(jqXHR, textStatus, errorThrown){
					//jQuery('#_errorResult').html('Ocurrio un error:<hr>'+jqXHR.responseText);
					jQuery('#_errorResult').html('<p class=\'auth-item-error-msg\'>no se pudo aplicar</p>');
					jQuery('#_errorResult').show('slow');
					setTimeout(function(){
						jQuery('#_errorResult').hide('slow');
						jQuery('#_errorResult').html('');
					},3000);
					loader.html('');
				},
			...
			var _pideBiz = function(data, textStatus, jqXHR){
					...
					if(resp){
					 // begin if resp
						...
						+'\"userid\": \"". $model->getPrimaryKey()."\" , \"bz\": \"'+resp+'\" }';	
						...
						var options2 = {
							...
							error: function(jqXHR, textStatus, errorThrown){
								jQuery('#_errorResult').html('<p class=\'auth-item-error-msg\'>no se pudo leer</p>');
								jQuery('#_errorResult').show('slow');
								setTimeout(function(){
									jQuery('#_errorResult').hide('slow');
									jQuery('#_errorResult').html('');
...
"; 
cs()->registerScript( 'crugeEditAdvancedProfile', $script );
?>
