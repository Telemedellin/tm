    ...
    function galleryManager(el, options) {
        ...
        function createEditorElement(id, src, name, description) {
            var html = '<div class="photo-editor">' +
                ...
                (opts.hasName
                    ? '<div class="form-group"><label for="photo_name_' + id + '">' + opts.nameLabel + ':</label>' +
                    '<input type="text" name="photo[' + id + '][name]" class="form-control" value="' + htmlEscape(name) + '" id="photo_name_' + id + '"/></div>'
                    : '') /*+
                (opts.hasDesc
                    ? '<div class="form-group"><label for="photo_description_' + id + '">' + opts.descriptionLabel + ':</label>' +
                    '<textarea name="photo[' + id + '][description]" rows="3" cols="40" class="form-control" id="photo_description_' + id + '">' + htmlEscape(description) + '</textarea></div>'
                    : '')/**/ +
                '</div>' +
                '</div>';
            return $(html);
        ...
        if (opts.hasName || opts.hasDesc)photoTemplate += '<span class="editPhoto btn btn-primary btn-mini"><span class="glyphicon glyphicon-pencil"></span></span> ';
        photoTemplate += '<span class="deletePhoto btn btn-danger btn-mini"><span class="glyphicon glyphicon-remove"></span></span>' +
        ...
        function editPhotos(ids) {
            ...
            for (var i = 0; i < l; i++) {
                ...
                form.append(createEditorElement(id, src, nombre, description));
            }
                ...
                for (var i = 0; i < filesCount; i++) {
                    ...
                    xhr.onload = function () {
                        ...
                        if (this.status == 200) {
                            ...
                            addPhoto(resp['id'], resp['preview'], resp['nombre'], resp['description'], resp['rank']);
                            ids.push(resp['id']);
                        } else {
                            // exception !!!
                        }
